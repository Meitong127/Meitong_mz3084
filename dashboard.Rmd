...
```{r}
ny_noaa |>
  mutate(text_label = paste("Max Temp: ", tmax, "°F", "\nMin Temp: ", tmin, "°F")) |>
  plot_ly(
    x = ~tmin,
    y = ~tmax,
    type = "scatter",
    ...
    marker = list(
      ...
      color = ~tmax,      
      colorscale = "Viridis", 
      colorbar = list(title = "Max Temp (°F)")
    ),
    text = ~text_label  
  ) |>
  layout(
    title = "Max Temperature vs Min Temperature",
    xaxis = list(title = "Min Temperature (°F)"),
...
ny_noaa |>
  ...
  layout(
    ...
    xaxis = list(title = "prcp group (mm)"),
...
plot_ly(
  ...
  layout(
    ...
    yaxis = list(title = "Highest Temperature (°F)")
  )
```
